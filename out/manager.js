...
class Manager {
    ...
    telemetryCollectors = [];
    userId = '';
    displayName = ''; // User's chosen display name for sessions
    sessionId = '';
    reconnectTimeout = null;
    isInspectorMode = false;
    isStudentMode = false;
    monitoringStatusBar = null;
    sessionTimer = null;
    sessionStartTime = 0;
    lastTelemetryTime = 0;
    lastFocusLostTime = 0;
    focusTrackingListener = null;
    MIN_TELEMETRY_INTERVAL = 100; // Max 10 events per second
    pendingInspectorSessionResolver = null;
    pendingInspectorSessionRejecter = null;
    pendingInspectorSessionTimer = null;
    activeSessionMembers = new Set();
    ...
    startStudentSession(sessionId, displayName) {
    ...
    showMonitoringNotification() {
        ...
        if (!this.monitoringStatusBar) {
            this.monitoringStatusBar = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Right, 1000 // High priority
            );
        ...
        vscode.window.showWarningMessage("🔍 Inspector Session Active: Your VS Code activity is being monitored for academic integrity purposes.", { modal: true }, // Requires explicit acknowledgment
        "I Understand").then(selection => {
    ...
    trackWindowFocus() {
        ...
        this.focusTrackingListener = vscode.window.onDidChangeWindowState(state => {
            ...
            if (this.sessionId && this.ws && this.ws.readyState === ws_1.default.OPEN) {
        ...
        this.telemetryCollectors.push(vscode.window.onDidChangeActiveTextEditor(editor => {
            if (editor) {
        ...
        this.telemetryCollectors.push(vscode.workspace.onDidSaveTextDocument(document => {
    ...
    calculateProductivityScore(sessionData) {
        ...
        const keystrokeEvents = telemetryData.filter((t) => t.type === 'keystroke_activity');
        ...
        const codePatternEvents = telemetryData.filter((t) => t.data.hasCodePatterns);
        ...
        const suspiciousEvents = telemetryData.filter((t) => t.data.suspiciousActivity);
        ...
        const pasteEvents = telemetryData.filter((t) => t.type === 'paste_detected');
        ...
        try {
            ...
            return new Promise((resolve, reject) => {
                ...
                this.ws.on('open', () => {
                    ...
                    this.sendMessage(protocol_1.Protocol.createUserUpdateMessage({
                        id: this.userId,
    ...
    startHeartbeat() {
        this.heartbeatInterval = setInterval(() => {
            if (this.ws && this.ws.readyState === ws_1.default.OPEN) {
                this.sendMessage(protocol_1.Protocol.createHeartbeatMessage(this.userId));
    ...
    handleMessage(data) {
        try {
            ...
            if (message.type === "problem.created") {
                ...
                vscode.window.showInformationMessage(`[Manager] Inspector session created: ${message.sessionId} (copied to clipboard)`);
                ...
                problems.forEach((p) => this.storage.saveProblem(p));
                suggestions.forEach((s) => this.storage.saveSuggestion(s));
    ...
    setupTelemetryCollection() {
        ...
        this.telemetryCollectors.push(vscode.workspace.onDidChangeTextDocument(event => {
        ...
        this.telemetryCollectors.push(vscode.window.onDidChangeTextEditorSelection(event => {
        ...
        this.telemetryCollectors.push(vscode.workspace.onDidOpenTextDocument(document => {
            if (this.getConfig().enableTelemetry) {
        ...
        // File close tracking
        this.telemetryCollectors.push(vscode.workspace.onDidCloseTextDocument(document => {
            if (this.getConfig().enableTelemetry) {
                this.collectTelemetry('file_activity', {
                    action: 'close',
                    file: document.uri.toString(),
        ...
        // Command execution
        this.telemetryCollectors.push(vscode.commands.registerCommand('conaint.telemetry.command', (commandId) => {
            if (this.getConfig().enableTelemetry) {
    ...
    collectTelemetry(type, data) {
        ...
        if (this.isStudentMode && this.connectionStatus.connected && this.ws && this.ws.readyState === ws_1.default.OPEN) {
    ...
    notifyActivityToInspector(type, data) {
    ...
    // Public API Methods
    async submitProblem(title, description, difficulty, tags) {
        const problem = {
            id: (0, uuid_1.v4)(),
            title,
            description,
        ...
        if (this.connectionStatus.connected) {
            this.sendMessage(protocol_1.Protocol.createProblemMessage(problem, this.userId));
        }
        return problem;
    }
    async submitSuggestion(problemId, content) {
        const suggestion = {
            id: (0, uuid_1.v4)(),
            problemId,
            content,
        ...
        await this.storage.saveSuggestion(suggestion);
        if (this.connectionStatus.connected) {
            this.sendMessage(protocol_1.Protocol.createSuggestionMessage(suggestion, this.userId));
        }
        return suggestion;
    }
    async startInspectorMode() {
        ...
        void this.notifyInspectorPanelOfSessionState();
        vscode.window.showInformationMessage(`Inspector mode started!\nSession ID: ${sessionId}\nShare this ID with students to join.`, 'Copy Session ID').then(selection => {
    ...
    async joinInspectorSession(sessionId, userMatricNo) {
        ...
        vscode.window.showInformationMessage(`✅ Successfully joined inspection session!\n` +
            `Session ID: ${sessionId}\n` +
            `Student ID: ${studentId}\n` +
            `⚠️ Your activity is now being monitored.`, 'Understood');
        // Update status bar to show monitoring status
        this.updateStatusBarForStudent(sessionId, studentId);
    }
    async leaveInspectorSession() {
        if (this.ws && this.ws.readyState === ws_1.default.OPEN && this.sessionId) {
            this.ws.send(JSON.stringify({
                type: 'session.leave',
                sessionId: this.sessionId,
                userId: this.userId,
    ...
    async requestAIAnalysis(code) {
        if (this.connectionStatus.connected) {
            this.sendMessage(protocol_1.Protocol.createAIAnalysisMessage(code, this.userId));
        }
        else {
    ...
    async notifyInspectorPanelOfSessionState() {
    ...
    updateStatusBarForStudent(sessionId, studentId) {
    ...
    async analyzeWithAI(sessionData, code = '') {
        try {
            const config = this.getConfig();
    ...
    detectSuspiciousPatterns(sessionData) {
        // Analyze for suspicious patterns
        const events = sessionData.events || [];
        const pasteEvents = events.filter((e) => e.type === 'keystroke_activity' &&
            e.data.suspiciousActivity);
        return {
            suspiciousPastes: pasteEvents.length,
            rapidTyping: events.filter((e) => e.type === 'keystroke_activity' &&
                e.data.totalChanges > 5).length,
            focusLoss: events.filter((e) => e.type === 'focus_change' &&
                !e.data.hasFocus).length
        };
    }
    analyzePastePatterns(sessionData) {
        const events = sessionData.events || [];
        const pasteEvents = events.filter((e) => e.type === 'keystroke_activity' && e.data.suspiciousActivity);
        return {
            totalPastes: pasteEvents.length,
            averagePasteLength: pasteEvents.length > 0 ?
                pasteEvents.reduce((sum, e) => sum + (e.data.changes[0]?.textLength || 0), 0) / pasteEvents.length : 0,
            codePatternPastes: pasteEvents.filter((e) => e.data.codePatterns).length
        };
    }
    analyzeFocusPatterns(sessionData) {
        const events = sessionData.events || [];
        const focusEvents = events.filter((e) => e.type === 'focus_change');
        return {
            totalFocusChanges: focusEvents.length,
            focusLossCount: focusEvents.filter((e) => !e.data.hasFocus).length,
            avgTimeAway: focusEvents.length > 0 ?
                focusEvents.reduce((sum, e) => sum + (e.data.timeAway || 0), 0) / focusEvents.length : 0
        };
    }
