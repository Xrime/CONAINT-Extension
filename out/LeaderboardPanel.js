...
class LeaderboardPanel {
    ...
    static createOrShow(extensionUri, manager, storage) {
        ...
        const panel = vscode.window.createWebviewPanel('leaderboard', 'CONAINT Leaderboard', column || vscode.ViewColumn.One, {
            enableScripts: true,
            localResourceRoots: [
...
Average Productivity: ${userAnalyses.length > 0 ?
    ...
    async _handleFilterChange(data) {
        ...
        const enhancedUsers = users.map(user => {
            ...
            return {
                ...
                enhancedStats: {
                    ...
                    lastActivity: Math.max(user.lastActivity, ...userProblems.map(p => p.timestamp), ...userSuggestions.map(s => s.timestamp), ...userAnalyses.map(a => a.timestamp))
                }
            };
        });
                ...
                <ul class="leaderboard-list">
                    ${sortedUsers.map((user, index) => `
                        <li class="leaderboard-item" onclick="viewProfile('${user.id}')">
                            ...
                            <div class="integrity-indicator ${user.enhancedStats.suspiciousCount === 0 ? 'integrity-good' :
            user.enhancedStats.suspiciousCount <= 2 ? 'integrity-warning' : 'integrity-alert'}" title="Academic Integrity Score"></div>
