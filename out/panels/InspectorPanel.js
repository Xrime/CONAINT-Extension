...
class InspectorPanel {
    static currentPanel;
    static viewType = 'conaint.inspectorPanel';
    _panel;
    _extensionUri;
    _disposables = [];
    manager;
    ...
    static createOrShow(extensionUri, manager) {
        ...
        const panel = vscode.window.createWebviewPanel(InspectorPanel.viewType, 'CONAINT Inspector', column || vscode.ViewColumn.Two, {
            enableScripts: true,
    ...
    async exportTelemetryData() {
        try {
            ...
            vscode.window.showInformationMessage(`Optimized telemetry exported: ${optimizedData.events.length} events (${Math.round((1 - optimizedData.events.length / rawTelemetryData.length) * 100)}% smaller) from ${rawTelemetryData.length} original records`);
        }
        catch (error) {
            vscode.window.showErrorMessage('Failed to export data: ' + error.message);
        }
    }
    optimizeTelemetryData(rawData) {
    ...
    extractCriticalData(eventType, data) {
    ...
    extractRelevantData(eventType, data) {
                ...
                try {
                    ...
                    try {
                        ...
                        try {
                            ...
                            const choice = await vscode.window.showErrorMessage('❌ Cannot connect to server. Would you like to start the local server?', 'Start Local Server', 'Check Server Status', 'Cancel');
                            if (choice === 'Start Local Server') {
                                vscode.window.showInformationMessage('🚀 To start the local server:\n' +
                                ...
                                this._panel.webview.postMessage({
                                    ...
                                    remote: `Failed: ${remoteError.message}`,
                                    instructions: 'Start local server with: cd server && node index.js'
                                });
                            }
                            throw new Error(`Both remote and local servers failed. Remote: ${remoteError.message}, Local: ${localError.message}`);
                        }
                    }
                    if (connected) {
