...
wss.on('connection', (ws) => {
  ...
  ws.on('message', (raw) => {
    ...
    if (data.type === 'session.leave') {
      const sid = data.sessionId || ws._meta.sessionId;
      if (!sid || !sessions[sid]) {
        console.log('[Server] session.leave ignored - unknown session', sid);
        return;
      }

      if (sessions[sid].members.has(ws)) {
        sessions[sid].members.delete(ws);
        ws._meta.sessionId = null;
        broadcastToInspector(sid, {
          type: 'member.left',
          userId: ws._meta.userId,
          displayName: ws._meta.displayName
        });
        send(ws, { type: 'left', sessionId: sid });
        console.log(`[Server] client ${ws._meta.userId} left session ${sid}`);
      }
      return;
    }
